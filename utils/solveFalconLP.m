function solution = solveFalconLP(LPproblem, varargin)
...
optParamNames = {'minNorm','printLevel','primalOnly','saveInput', ...
    ...
    'EqtNameFun','VarNameFun','PbName','MPSfilename','GurobiParams'};
parameters = '';
if nargin ~=1
...
switch solver
	...
	vidx = find(strcmp(varargin,'GurobiParams'));
	params = struct;
	if vidx 
          params = varargin{vidx(1)+1};
	  if isfield(params,'vbasis')
	    LPproblem.vbasis = params.vbasis;
	    params = rmfield(params,'vbasis');
	  end
	  if isfield(params,'cbasis')
	    LPproblem.cbasis = params.cbasis;
	    params = rmfield(params,'cbasis');
	  end
	end
        if printLevel == 0 
           params.OutputFlag = 0;
	...
	if (printLevel > 0 && ~strcmp(resultgurobi.status,'OPTIMAL'))
	  errfid = ['gurobi5_sol_',num2str(randint(1,1,10e30)),'.mat'];
	  disp(['Saving error info to: ', errfid]);
	  save(errfid,'resultgurobi','LPproblem');
	end

        if strcmp(resultgurobi.status,'OPTIMAL')
           stat = 1; % Optimal solution found
           [x,f,y,basis,cbasis] = deal(resultgurobi.x,resultgurobi.objval, ...
		resultgurobi.pi,resultgurobi.vbasis,resultgurobi.cbasis);
...
if ~strcmp(solver,'cplex_direct') && ~strcmp(solver,'mps')
    ...
    if ~exist('cbasis','var'), cbasis=[]; end
    [solution.full,solution.obj,solution.rcost,solution.dual,solution.solver,solution.stat,solution.origStat,solution.time,solution.basis,solution.cbasis] = ...
        deal(x,f,w,y,solver,stat,origStat,t,basis,cbasis);
end
